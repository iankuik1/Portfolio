...
// Generate mines at random locations within grid
...
function init(cell) {
    ...
    if (lockGame) {
        ...
        if (cell.getAttribute("mine") == "true") {
            ...
            cell.innerHTML = mineCount; // show number of adjacent mines in grid
            if (mineCount == 0) {
                // if cell doesn't have mine, check surrounding cells
                for (var i = Math.max(cellRow - 1, 0); i <= Math.min(cellRow + 1, 9); i++) {
                    for (var j = Math.max(cellCol - 1, 0); j <= Math.min(cellCol + 1, 9); j++) {
